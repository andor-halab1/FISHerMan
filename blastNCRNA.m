% blast oligos against ncrna database. Only blast against the complementary 
% sequences of ncrna. Rule out the possibility of homology of 15 nt or more.

function [Header,Sequence,nonSequence]=blastNCRNA(Header,Sequence,nonSequence,varargin)

if length(varargin) >= 1
    params = varargin{1};
else
    params = struct('species','Mouse','verbose',1,...
        'thres',30,'querySize',30,'DbSize',10^7,'seqNum',1000,...
...
if params.verbose
    disp('removing non-specific oligos against the ncrna database');
end
...
%% Find out the oligos that non-specifically hit ncrna database
seqDelete = [];
for n = 1:length(data)
